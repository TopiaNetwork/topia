...
func Test_transactionPool_truncateQueue(t *testing.T) {
	...
	assert.Equal(t, 384, len(pool.queues.queue[Category1]))
	assert.Equal(t, 0, len(pool.pendings.pending[Category1]))
	...
	assert.Equal(t, 256, len(pool.queues.queue[Category1]))
	assert.Equal(t, 0, len(pool.pendings.pending[Category1]))
	assert.Equal(t, 200, pool.allTxsForLook.all[Category1].LocalCount())
...
func Test_transactionPool_truncatePending(t *testing.T) {
	...
	for i := 1; i <= 200; i++ {
		...
		assert.Equal(t, 0, pool.queues.queue[Category1][fromlocal].txs.Len())
		assert.Equal(t, 0, pool.queues.queue[Category1][fromremote].txs.Len())
	}
	assert.Equal(t, 384, len(pool.queues.queue[Category1]))
	assert.Equal(t, 384, len(pool.pendings.pending[Category1]))
	assert.Equal(t, 200, pool.allTxsForLook.all[Category1].LocalCount())
	assert.Equal(t, 184, pool.allTxsForLook.all[Category1].RemoteCount())
