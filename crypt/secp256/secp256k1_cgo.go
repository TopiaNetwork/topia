...
func contextRandomize(ctx secp256k1ContextPointer, random []byte) error {
	...
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_context_randomize(ctx.p, (*C.uchar)(unsafe.Pointer(&random[0]))))) {
...
func ecSeckeyVerify(ctx secp256k1ContextPointer, seckey []byte) (bool, error) {
	...
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ec_seckey_verify(ctx.p, (*C.uchar)(unsafe.Pointer(&seckey[0]))))) {
		return false, nil
	}
	return true, nil
}
...
func ecPubkeyCreateAndSerialize(ctx secp256k1ContextPointer, seckey []byte) (pubkey [PublicKeyBytes]byte, err error) {
	...
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ec_pubkey_create(ctx.p, &pub.v, (*C.uchar)(unsafe.Pointer(&seckey[0]))))) { // != 1
		return [65]byte{}, errors.New("failed to create pubkey")
	}

	pubkeyLen := uint(PublicKeyBytes)
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ec_pubkey_serialize(ctx.p, (*C.uchar)(unsafe.Pointer(&pubkey[0])), (*C.size_t)(unsafe.Pointer(&pubkeyLen)), &pub.v, C.SECP256K1_EC_UNCOMPRESSED))) {
...
func ecdsaSignAndSerialize(ctx secp256k1ContextPointer, seckey []byte, msg []byte) (serializedSig [SignatureRecoverableBytes]byte, err error) {
	...
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ecdsa_sign_recoverable(ctx.p, &recSig.v, (*C.uchar)(unsafe.Pointer(&msg[0])), (*C.uchar)(unsafe.Pointer(&seckey[0])), C.secp256k1_nonce_function_rfc6979, nil))) {
		return [SignatureRecoverableBytes]byte{}, errors.New("failed to sign")
	}
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ecdsa_recoverable_signature_serialize_compact(ctx.p, (*C.uchar)(unsafe.Pointer(&serializedSig[0])), (*C.int)(&recID), &recSig.v))) {
...
func ecdsaVerify(ctx secp256k1ContextPointer, pubkey []byte, signData []byte, msg []byte) (bool, error) {
	...
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ecdsa_signature_parse_compact(ctx.p, &sig.v, (*C.uchar)(unsafe.Pointer(&signData[0]))))) {
	...
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ecdsa_verify(ctx.p, &sig.v, (*C.uchar)(unsafe.Pointer(&msg[0])), &pub.v))) {
...
func ecdsaRecoverPubkey(ctx secp256k1ContextPointer, msg []byte, signData []byte) (pubkey [PublicKeyBytes]byte, err error) {
	...
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ecdsa_recoverable_signature_parse_compact(ctx.p, (*C.secp256k1_ecdsa_recoverable_signature)(recSigPointer), (*C.uchar)(unsafe.Pointer(&signData[0])), C.int(int32(signData[SignatureRecoverableBytes-1]))))) {
		return [PublicKeyBytes]byte{}, errors.New("signature could not be parsed to recoverable signature")
	}
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ecdsa_recover(ctx.p, (*C.secp256k1_pubkey)(pubkeyPointer), (*C.secp256k1_ecdsa_recoverable_signature)(recSigPointer), (*C.uchar)(unsafe.Pointer(&msg[0]))))) {
		return [PublicKeyBytes]byte{}, errors.New("public key failed to recover")
	}
	pubkeyLen := uint(PublicKeyBytes)
	if !(C._Bool)(C.intToBoolSecp256(C.secp256k1_ec_pubkey_serialize(ctx.p, (*C.uchar)(unsafe.Pointer(&pubkey[0])), (*C.size_t)(unsafe.Pointer(&pubkeyLen)), (*C.secp256k1_pubkey)(pubkeyPointer), C.SECP256K1_EC_UNCOMPRESSED))) {
