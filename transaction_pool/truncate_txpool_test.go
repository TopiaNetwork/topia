...
func Test_transactionPool_truncateQueue(t *testing.T) {
	...
	for i := 1; i <= 200; i++ {
		...
		assert.Equal(t, 1, pool.queues.getTxsByCategory(Category1).getTxListByAddr(fromlocal).txs.Len())
		_ = pool.AddTx(txremote, false)
		fromremote = tpcrtypes.Address(txremote.Head.FromAddr)
		assert.Equal(t, 1, pool.queues.getTxsByCategory(Category1).getTxListByAddr(fromremote).txs.Len())
		//fmt.Printf("i:%d,", i)
		//fmt.Println("len(pool.queues[Category1].accTxs)", len(pool.queues[Category1].accTxs))
	}
	assert.Equal(t, 384, len(pool.queues.getTxsByCategory(Category1).getAll()))
	assert.Equal(t, 0, len(pool.pendings.getTxsByCategory(Category1).getAll()))
	...
	assert.Equal(t, 256, len(pool.queues.getTxsByCategory(Category1).getAll()))
	assert.Equal(t, 0, len(pool.pendings.getTxsByCategory(Category1).getAll()))
	assert.Equal(t, 200, pool.allTxsForLook.all[Category1].LocalCount())
...
func Test_transactionPool_truncatePending(t *testing.T) {
	...
	for i := 1; i <= 200; i++ {
		...
		assert.Equal(t, 0, pool.queues.getTxsByCategory(Category1).getTxListByAddr(fromlocal).txs.Len())
		assert.Equal(t, 0, pool.queues.getTxsByCategory(Category1).getTxListByAddr(fromremote).txs.Len())
	}
	assert.Equal(t, 384, len(pool.queues.getTxsByCategory(Category1).getAll()))
	assert.Equal(t, 384, len(pool.pendings.getTxsByCategory(Category1).getAll()))
	assert.Equal(t, 200, pool.allTxsForLook.all[Category1].LocalCount())
	assert.Equal(t, 184, pool.allTxsForLook.all[Category1].RemoteCount())
