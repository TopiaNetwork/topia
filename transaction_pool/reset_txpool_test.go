...
func Test_transactionPool_Reset(t *testing.T) {
	...
	servant.EXPECT().GetBlock(gomock.Eq(types.BlockHash(hex.EncodeToString(OldBlockHead.Hash))), OldBlockHead.Height).
		Return(OldBlock).AnyTimes()
	servant.EXPECT().GetBlock(gomock.Eq(types.BlockHash(hex.EncodeToString(NewBlockHead.Hash))), NewBlockHead.Height).
